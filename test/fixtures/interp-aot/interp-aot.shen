\\ interp-aot: interpreter.shen shaken to eval-free NativeFunctions (no load).
\\ Datatypes/signatures are host-compiled away; runtime is normal-form only.
...
\\ AOT driver: interpreter is shaken to KL/NativeFunctions, not (load).
(output "normal-form = ~A~%" (normal-form [[[y-combinator [/. ADD [/. X [/. Y [if [= X 0] Y [[ADD [-- X]] [++ Y]]]]]]] 3] 4]))
(output "inferences = ~A~%" (inferences))
